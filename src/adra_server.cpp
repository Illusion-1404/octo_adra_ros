...
constexpr unsigned int hash(const char *s, int off = 0) { return !s[off] ? 5381 : (hash(s, off + 1) * 33) ^ s[off]; } // const expression hash function for switch with string .
// https://newbedev.com/why-the-switch-statement-cannot-be-applied-on-strings
...
void connect(adra_ros::Api::Request &req, adra_ros::Api::Response &res) {
  ...
  int id = std::stoi(req.args[1].c_str());   // convert string to integer using std::stoi(str,position,int base)
